...
use app\modules\pctclinic\models\SmokingType;
...
$form->field($model, 'clinic')->widget(Select2::className(), ['data' =>
    ArrayHelper::map(app\modules\pctclinic\models\Clinic::find()->orderBy('clinic')->all(), 'clinic', 'name'),
    'options' => [
        'placeholder' => 'คลินิก'],
    'pluginOptions' =>
    [
...
<?=
$form->field($model, 'smoke')->label('บุหรี่')->widget(Select2::className(), ['data' =>
    ArrayHelper::map(SmokingType::find()->orderBy('smoking_type_id')->all(), 'smoking_type_id', 'smoking_type_name'),
    'options' => [
        'placeholder' => '-'],
    'pluginOptions' =>
    [
...
<?= $form->field($model, 'hosconfirm')->label('รับเคสเข้าระบบ')->radioList(\app\modules\pctclinic\models\PctClinicVisit::itemAlias('hosconfirm')) ?>            
        </div>
        <div class="col-xs-3 col-sm-3 col-md-4">            
<?= $form->field($model, 'sendto')->label('<i class="glyphicon glyphicon-pushpin"></i> ผลการประเมิน')->radioList(\app\modules\pctclinic\models\PctClinicVisit::itemAlias('sendto')) ?>
        </div> 
        <div class="col-xs-3 col-sm-3 col-md-3">            
<?= $form->field($model, 'ssoconfirm')->label('<i class="glyphicon glyphicon-ok-sign"></i>ยืนยันรับเคส')->radioList(\app\modules\pctclinic\models\PctClinicVisit::itemAlias('ssoconfirm')) ?>
        </div>
    </div>
